              ...
              {section.links.map((link) => (
                <li key={link.href} className="relative">
                  <Link
                    ...
                    onClick={onLinkClick}
                    className={clsx(
                      'block w-full pl-3.5 before:pointer-events-none before:absolute before:top-1/2 before:-left-1 before:h-1.5 before:w-1.5 before:-tranzinc-y-1/2 before:rounded-full',
                      link.href === pathname
                        ? 'font-semibold text-amber-500 before:bg-amber-500'
