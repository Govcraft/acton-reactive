...
export function QuickLinks({ children }: { children: React.ReactNode }) {
  return (
    <div className="not-pamber my-12 grid grid-cols-1 gap-6 sm:grid-cols-2">
      {children}
    </div>
  )
}
...
export function QuickLink({
  ...
  return (
    <div className="group relative rounded-xl border border-zinc-200 dark:border-zinc-800">
      <div className="absolute -inset-px rounded-xl border-2 border-transparent opacity-0 [background:linear-gradient(var(--quick-links-hover-bg,var(--color-amber-50)),var(--quick-links-hover-bg,var(--color-amber-50)))_padding-box,linear-gradient(to_top,var(--color-orange-400),var(--color-orange-400),var(--color-amber-500))_border-box] group-hover:opacity-100 dark:[--quick-links-hover-bg:var(--color-zinc-800)]" />
