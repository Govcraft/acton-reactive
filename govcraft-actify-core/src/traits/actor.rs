...
use std::sync::Arc;
use anyhow::Result;
use async_trait::{async_trait, async_trait as govcraft_async};
use tokio::time::{Duration, timeout};
use tracing::{debug, error, info, instrument, warn};
// use govcraft_actify_core::ActorSupervisorMessage;
...
pub trait GovcraftActor {
    ...
    async fn handle_supervisor_message(&mut self, _message: SupervisorMessage) -> anyhow::Result<()>{
        match _message {
            SupervisorMessage::Start => {
                info!("Supervisor says Start");
            }
            SupervisorMessage::Shutdown => {
                warn!("Supervisor says Stop");
            }
        }
        Ok(())
    }
